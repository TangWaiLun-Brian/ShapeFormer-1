...
class PartNetDataset(Dataset):
    def __init__(self, dataset='IMNet2_packed', cate="all", zoomfac=1,
                 ...
                 Xbd_as_Xct=False, Xct_as_Xbd=False,
                 partial_opt={"class": "shapeformer.data.partial.BallSelector",
                              "kwargs": dict(radius=.4, context_N=512)}):
        self.__dict__.update(locals())
        self.split = split
        self.grid_dim = grid_dim
        self.partnet_root = "/data/brian22/ShapeFormer/datasets/partnet_mount/"
        if cate == 'all':
            print('Category is set to all')
            print('Exit...')
            exit()
        else:
            self.cate = cate
            split_root = "/data/brian22/ShapeFormer/datasets/PartNet_Split/"
            split_path = os.path.join(split_root, f'partnet_{cate}_{split}.json')
            with open(split_path, 'r') as fp:
                self.shape_names = json.load(fp)
        ...
        self.length = len(self.shape_names)
        if split != "train":
    ...
    def __getitem__(self, index, all_target=False):
        ...
        o_ind = index
        name = self.shape_names[index]
        #with h5py.File(self.dpath, 'r') as f:
        Xbd = trimesh.load(os.path.join(self.partnet_root, self.split, self.cate, name, 'gt.ply'))
        ...
        id = np.random.randint(0, 10) if self.split == 'train' else 0
        Xct = trimesh.load(os.path.join(self.partnet_root, self.split, self.cate, name, f'partial_{id}.ply'))
        ...
        item = dict(Xct=Xct,
                    Xbd=Xbd,
                    Xtg=torch.from_numpy(Xtg).cpu().numpy(),
                    Ytg=torch.from_numpy(Ytg).cpu().numpy(),
                    index=torch.from_numpy(np.array(index)).long().cpu().numpy(),
                    )
        # print(item)
    ...
    def unittest(cls, **kwargs):
        ...
        imgs = [
            npfvis.plot_3d_sample(Xct=ditem["Xct"], Yct=None, Xtg=ditem["Xtg"], Ytg=ditem["Ytg"], pred_y=ditem["Ytg"],
...
class PartNetLowResDataset(Dataset):
    def __init__(self, dataset='IMNet2_64', cate="all", zoomfac=1,
                 duplicate_size=1, split='train', boundary_N=2048, target_N=-1, grid_dim=64, multi_process=0, process_number=0, weighted_sampling=False,
                 Xbd_as_Xct=False, resume_dir=None, pre=0, Xct_as_Xbd=False,
                 partial_opt={"class": "shapeformer.data.partial.BallSelector",
                              "kwargs": dict(radius=.4, context_N=512)}):
        self.__dict__.update(locals())
        self.split = split
        ...
        self.dpath = dpath = f'/data/brian22/ShapeFormer/datasets/{"IMNet2_packed"}/{"train"}.hdf5'  # f'datasets/{dataset}/{split}.hdf5'
        self.dpath2 = f'/data/brian22/ShapeFormer/datasets/{"IMNet2_packed"}/{"test"}.hdf5'  # f'datasets/{dataset}/{split}.hdf5'
        self.weighted_sampling = weighted_sampling
        self.grid_dim = grid_dim
        self.partnet_root = "/data/brian22/ShapeFormer/datasets/partnet_mount/"
        if cate == 'all':
        ...
        if multi_process > 0:
            self.shape_names = self.shape_names[(len(self.shape_names) - pre) * process_number // multi_process + pre:(len(self.shape_names) - pre) * (process_number + 1) // multi_process + pre]
            self.shape_names = [name for name in self.shape_names if
                                not os.path.exists(os.path.join(resume_dir, name['model_id'], 'fake-z9.obj'))]
        ...
        self.Xbds_0 = nputil.H5Var(self.dpath, "Xbd")
        self.Ytgs_0 = nputil.H5Var(self.dpath, "Ytg")  # packed

        self.Xbds_1 = nputil.H5Var(self.dpath2, "Xbd")
        self.Ytgs_1 = nputil.H5Var(self.dpath2, "Ytg")  # packed
        ...
        self.imnet_fixed_path = os.path.join("/data/brian22/ShapeFormer/datasets/IMNet2_fixed_data/", self.split)
        # print(self.imnet_fixed_path)
        lines = open(imnet_path + f"/shape_name_{'train'}.txt", "r").readlines()
        train_shape_names = [line.strip().split('/')[-1] for line in lines]
        
        lines = open(imnet_path + f"/shape_name_{'test'}.txt", "r").readlines()
        test_shape_names = [line.strip().split('/')[-1] for line in lines]

        train_index_list = []
        test_index_list = []
        map_mode = [[], []]
        map_split = []
        id_only = [itm['model_id'] for itm in self.shape_names]
        for i in range(len(self.shape_names)):
            try:
                train_index_list.append(train_shape_names.index(self.shape_names[i]['model_id']))
                map_mode[0].append(i)
                map_split.append(0)
            except:
                test_index_list.append(test_shape_names.index(self.shape_names[i]['model_id']))
                map_mode[1].append(i)
                map_split.append(1)
        self.map_split = map_split

        # category = 0 if cate == '03001627' else 1
        # dpath = f'/data/brian22/ShapeFormer/datasets/{"IMNet2_packed"}/{"train"}.hdf5'
        # with h5py.File(self.dpath, 'r') as f:
        #     self.subset = np.array(f[f"cate_{category}"])
        # dpath = f'/data/brian22/ShapeFormer/datasets/{"IMNet2_packed"}/{"test"}.hdf5'
        # with h5py.File(self.dpath2, 'r') as f:
        #     self.subset2 = np.array(f[f"cate_{category}"])

        self.map_index = np.zeros((self.length,))
        self.map_index[map_mode[0]] = np.array(train_index_list)
        self.map_index[map_mode[1]] = np.array(test_index_list)
    ...
    def __getitem__(self, index, all_target=False):
        index = index % self.length
        o_ind = index
        name = self.shape_names[index]['model_id']
        # print(index, o_ind)
        ...
        x_dim = Xbd.shape[-1]
        #Ytg = np.zeros((64, 64, 64)).reshape(-1, 1)
        if not self.map_split[index]:
            mapped_index = int(self.map_index[index])
            with h5py.File(self.dpath, 'r') as f:
                Ytg = np.unpackbits(self.Ytgs_0[mapped_index], axis=-1)[..., None]  # index
        else:
            mapped_index = int(self.map_index[int(index)])
            with h5py.File(self.dpath2, 'r') as f:
                Ytg = np.unpackbits(self.Ytgs_1[mapped_index], axis=-1)[..., None]  # index
